...
<body>
	<div class="ibody">
		...
		<a href="statisticServlet">数据统计</a></nav> </header>
		<article>
		<div class="banner">
			<ul class="texts">
		...
		<div class="bloglist">
			...
			<%
				...
				ArticleDAO arcDao = new ArticleDAO();
				List arList = arcDao.findFirst();
				for (int i = 0; i < 5; i++) {
					Article article = (Article) arList.get(i);
			%>
			<div class="blogs">
				<h3>
		...
		<div class="tj_news">
			...
			<%							
				for (int i = 0; i < 5; i++) {
					Article article = (Article) arList.get(i);
			%>
			<li><a href="ArticleDetailServlet?id=<%=article.getId() %>"><%=article.getTitle()%></a></li>
			<%
				}
			%>
